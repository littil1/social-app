...
update public.daily_post_winners
set boost_count = coalesce(boost_counts.count, 0)
from (
  select
    post_boosts.post_id,
...
where daily_post_winners.post_id = boost_counts.post_id;
...
begin
  ...
  ranked_posts as (
    select
      ...
      coalesce(boost_counts.boost_count, 0) as boost_count,
      (
        (
  ...
  if inserted_author_id is null then
    return jsonb_build_object(
      ...
      'winner', jsonb_build_object(
        ...
        'boost_count', winner_record.boost_count,
